...
class BriskGenerator(object):
    """Builds the brisk python package based on a released version of the
    open-traffic-generator openapi.yaml file.
    """
    def __init__(self, dependencies=True, openapi_filename=None, output_dir=None):
        ...
        #\Ixload OpenAPi\Ixload_backup_latest\Ixload backup\models
        #C:\Users\waseebai\Documents\laptop backup\work\Ixload OpenAPi\Ixload_backup_latest\Ixload backup\models
        self._openapi_filename = 'C://Users//waseebai//Documents//laptop backup//work//Ixload OpenAPi//Ixload_backup_latest//Ixload backup//models//openapi.yaml'
        self.__python = os.path.normpath(sys.executable)
        self.__python_dir = os.path.dirname(self.__python)
        self._src_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'brisk')
        self._output_dir = self._src_dir
        self._output_file = 'briskIxload'
        if output_dir != None:
    ...
    def _clean(self):
        ...
        - Remove any locally installed version of brisk
        - Remove generated files
        - Leave infrastructure files that are prefixed with the word brisk
        """
        process_args = [
    ...
    def _generate(self):
        self._api_filename = os.path.join(self._output_dir, 'briskIxload.py')
        ...
        with open(self._api_filename, 'w') as self._fid:
            ...
            self._write(1, 'pass')
            self._write(0, 'from .briskcommon import BriskObject')
            self._write(0, 'from .briskcommon import BriskIter')
            self._write(0, 'from .briskcommon import BriskHttpTransport')
    ...
    def _get_methods_and_factories(self):
        ...
        for ref, property_name in self._top_level_schema_refs:
            if property_name is None:
                self._write_brisk_object(ref)
            else:
                self._write_brisk_list(ref, property_name)
    ...
    def _write_api_class(self, methods, factories):
        ...
        with open(self._api_filename, 'a') as self._fid:
            ...
            self._write(1, '"""%s' % 'Brisk Abstract API')
            self._write(1, '"""')
            self._write()
            self._write(1, 'def __init__(self, host=None):')
    ...
    def _write_api_factory(self):
        ...
        with open(self._api_filename, 'a') as self._fid:
            self._write()
            self._write()
            self._write(0, 'def api(host=None, version=None):')
            self._write(1, 'if version is None:')
            self._write(2, 'return HttpApi(host=host)')
            self._write(0)
            self._write(1, 'try:')
            self._write(
                2, 'lib = importlib.import_module("brisk_ixload")'
            )
            self._write(
                2, 'return lib.Api(host=host, ixload_version=version)'
            )
    ...
    def _write_brisk_object(self, ref, choice_method_name=None):
        schema_object = self._get_object_from_ref(ref)
        ...
        with open(self._api_filename, 'a') as self._fid:
            self._write()
            self._write()
            self._write(0, 'class %s(BriskObject):' % class_name)
            slots = ''
        ...
        for ref in refs:
            self._write_brisk_object(ref[0], ref[3])
            if ref[1] is True:
                self._write_brisk_list(ref[0], ref[2])
    ...
    def _process_properties(self, class_name=None, schema_object=None, choice_child=False):
        ...
        if 'properties' in schema_object:
            ...
            for property_name in schema_object['properties']:
                ...
                self._write_brisk_property(schema_object, property_name, property, write_set_choice)
    ...
    def _write_brisk_list(self, ref, property_name):
        ...
        with open(self._api_filename, 'a') as self._fid:
            self._write()
            self._write()
            self._write(0, 'class %s(BriskIter):' % class_name)
            ...
            self._write_brisklist_special_methods(contained_class_name, yobject)
    ...
    def _write_brisklist_special_methods(self, contained_class_name, schema_object):
            ...
            for name, property in yobject['properties'].items():
                ...
                if 'obj' not in type_string:
                    ...
                    if 'enum' in property or 'format' in property:
                        ...
                        #import pdb; pdb.set_trace()
                        if type_string == "str" and name == "url":
    ...
    def _write_brisk_property(self, schema_object, name, property, write_set_choice=False):
        ref = parse("$..'$ref'").find(property)
        restriction = self._get_type_restriction(property)
        if len(ref) > 0:
    ...
    def _get_brisk_types(self, yobject):
        types = []
        if 'properties' in yobject:
...
if __name__ == '__main__':
    ...
    BriskGenerator(dependencies=False, openapi_filename=openapi_filename, output_dir=output_dir)

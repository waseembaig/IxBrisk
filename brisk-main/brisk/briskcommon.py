...
class BriskHttpTransport(object):
    ...
    def send_recv(self, method, relative_url, payload=None, return_object=None):
        ...
        if payload is not None:
            if isinstance(payload, (str, unicode)):
                data = payload
            elif isinstance(payload, BriskBase):
...
class BriskBase(object):
    ...
    def serialize(self, encoding=JSON):
        ...
        """
        if encoding == BriskBase.JSON:
            return json.dumps(self._encode(), indent=2)
        elif encoding == BriskBase.YAML:
            return yaml.safe_dump(self._encode())
        elif encoding == BriskBase.DICT:
            return self._encode()
        else:
    ...
    def deserialize(self, serialized_object):
        """Deserialize a python object into the current brisk object.

        If the input `serialized_object` does not match the current 
        brisk object an exception will be raised. 
        ...
        Returns
        -------
        - obj(briskcommon.BriskObject): This brisk object with all the 
...
class BriskObject(BriskBase):
    """Base class for any /components/schemas object

    Every BriskObject is reuseable within the schema so it can 
    ...
    def __init__(self, parent=None, choice=None):
        super(BriskObject, self).__init__()
        self._parent = parent
        self._choice = choice
        self._properties = {}
    ...
    def _encode(self):
        ...
        for key, value in self._properties.items():
            if isinstance(value, (BriskObject, BriskIter)):
    ...
    def _decode(self, obj):
        brisk_names = dir(self)
        for property_name, property_value in obj.items():
            if property_name in brisk_names:
                if isinstance(property_value, dict):
                    ...
                    for item in property_value:
                        ...
                        brisk_list._items.append(item)
                    property_value = brisk_list
...
class BriskIter(BriskBase):
    """Container class for BriskObject
    ...
    def append(self, item):
        """Append an item to the end of the BriskIter
    ...
    def _decode(self, encoded_brisk_list):
        item_class_name = self.__class__.__name__.replace('Iter', '')
        module = importlib.import_module(self.__module__)
        object_class = getattr(module, item_class_name)
        self.clear()
        for item in encoded_brisk_list:
            self._add(object_class()._decode(item))

    def __copy__(self):
        raise NotImplementedError('Shallow copy of BriskIter objects is not supported')

    def __deepcopy__(self, memo):
        raise NotImplementedError('Deep copy of BriskIter objects is not supported')
